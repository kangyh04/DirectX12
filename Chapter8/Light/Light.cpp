...
public:
	...
	LightApp(const LightApp& rhs) = delete;
	LightApp& operator=(const LightApp& rhs) = delete;
	~LightApp();
...
private:
	...
	FrameResource* mCurrentFrameResource = NULL;
	int mCurrentFrameResourceIndex = 0;
	UINT mCbvSrvDescriptorSize = 0;

	ComPtr<ID3D12RootSignature> mRootSignature = NULL;
	ComPtr<ID3D12DescriptorHeap> mSrvDescriptorHeap = NULL;
	...
	ComPtr<ID3D12PipelineState> mOpaquePSO = NULL;
...
LightApp::LightApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
...
bool LightApp::Initialize()
{
	...
	ThrowIfFailed(mCommandList->Reset(mDirectCmdListAlloc.Get(), NULL));
...
void LightApp::Update(const GameTimer& gt)
{
	...
	{
		HANDLE eventHandle = CreateEventExA(NULL, NULL, NULL, EVENT_ALL_ACCESS);
...
{
	auto cmdListAlloc = mCurrentFrameResource->ComdListAlloc;
	...
	mCommandList->ClearRenderTargetView(CurrentBackBufferView(), Colors::LightSteelBlue, 0, NULL);
	mCommandList->ClearDepthStencilView(DepthStencilView(), D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
...
void LightApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void LightApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void LightApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void LightApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void LightApp::BuildRootSignature()
{
	...
	CD3DX12_ROOT_SIGNATURE_DESC rootSigDesc(3, slotRootParameter, 0, NULL, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);

	ComPtr<ID3DBlob> serializedRootSig = NULL;
...
void LightApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Default.hlsl", NULL, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Default.hlsl", NULL, "PS", "ps_5_1");
...
void LightApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData box = geoGen.CreateBox(15.f, 0.5f, 1.5f, 3);
