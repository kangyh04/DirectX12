...
private:
	...
	void OnKeyboardInput(const Timer& t);
...
void TexColumnsApp::Update(const Timer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void TexColumnsApp::Draw(const Timer& gt)
{
	auto cmdListAllock = mCurrFrameResource->CmdListAlloc;

	ThrowIfFailed(cmdListAllock->Reset());

	ThrowIfFailed(mCommandList->Reset(cmdListAllock.Get(), mPSOs["opaque"].Get()));
...
void TexColumnsApp::OnKeyboardInput(const Timer& gt)
{

}

void TexColumnsApp::UpdateCamera(const Timer& gt)
{
...
void TexColumnsApp::AnimateMaterials(const Timer& gt)
{

}

void TexColumnsApp::UpdateObjectCBs(const Timer& gt)
{
...
void TexColumnsApp::UpdateMaterialCBs(const Timer& gr)
{
...
void TexColumnsApp::UpdateMainPassCB(const Timer& gt)
{
...
{
	const D3D_SHADER_MACRO alphaTextDefines[] =
	{
		"ALPHA_TEXT", "1", NULL, NULL
	};
...
void TexColumnsApp::BuildFrameResources()
{
	...
	{
		mFrameResources.push_back(make_unique<FrameResource>(md3dDevice.Get(), 1, (UINT)mAllRitems.size(), (UINT)mMaterials.size()));
	}
}
...
void TexColumnsApp::BuildRenderItems()
{
	...
	XMMATRIX bricksTexTransform = XMMatrixScaling(1.0f, 1.0f, 1.0f);
	UINT objCBIndex = 2;
	for (int i = 0; i < 5; ++i)
	{
		...
		XMStoreFloat4x4(&leftCylRitem->TexTransform, bricksTexTransform);
		leftCylRitem->ObjCBIndex = objCBIndex++;
		leftCylRitem->Mat = mMaterials["bricks0"].get();
		leftCylRitem->Geo = mGeometries["shapeGeo"].get();
		leftCylRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP;
		leftCylRitem->IndexCount = leftCylRitem->Geo->DrawArgs["cylinder"].IndexCount;
		leftCylRitem->StartIndexLocation = leftCylRitem->Geo->DrawArgs["cylinder"].StartIndexLocation;
		leftCylRitem->baseVertexLocation = leftCylRitem->Geo->DrawArgs["cylinder"].BaseVertexLocation;

		XMStoreFloat4x4(&rightCylRitem->World, rightCylWorld);
		XMStoreFloat4x4(&rightCylRitem->TexTransform, bricksTexTransform);
		rightCylRitem->ObjCBIndex = objCBIndex++;
		rightCylRitem->Mat = mMaterials["bricks0"].get();
		rightCylRitem->Geo = mGeometries["shapeGeo"].get();
		rightCylRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP;
		...
		leftSphereRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP;
		...
		rightSphereRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP;
...
void TexColumnsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); ++i)
	{
		...
		cmdList->DrawIndexedInstanced(ri->IndexCount, ri->StartIndexLocation, ri->baseVertexLocation);
	}
}

array<const CD3DX12_STATIC_SAMPLER_DESC, 6> TexColumnsApp::GetStaticSamplers()
{
