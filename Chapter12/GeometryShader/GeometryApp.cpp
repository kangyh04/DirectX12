...
#include "LandUtility.h"

class GeometryApp : public BaseApp
{
public:
...
void GeometryApp::Update(const Timer& gt)
{
	BaseApp::Update(gt);
	UpdateWaves(gt);
}

void GeometryApp::OnKeyboardInput(const Timer& gt)
{
}

void GeometryApp::AnimateMaterials(const Timer& gt)
{
...
void GeometryApp::UpdateWaves(const Timer& gt)
{
...
void GeometryApp::BuildRootSignature()
{
	...
	rootParameters[texRootParameterIndex].InitAsDescriptorTable(1, &texTable, D3D12_SHADER_VISIBILITY_PIXEL);
	rootParameters[objRootParameterIndex].InitAsConstantBufferView(0);
	rootParameters[passCBRootParameterIndex].InitAsConstantBufferView(1);
	rootParameters[matCBRootParameterIndex].InitAsConstantBufferView(2);
...
void GeometryApp::BuildLandGeometry()
{
	...
	for (size_t i = 0; i < grid.Vertices.size(); ++i)
	{
		...
		vertices[i].Pos = p;
		vertices[i].Pos.y = LandUtility::GetHillsHeight(p.x, p.z);
		vertices[i].Normal = LandUtility::GetHillsNormal(p.x, p.z);
...
void GeometryApp::BuildWavesGeometry()
{
...
void GeometryApp::BuildTreeSpritesGeometry()
{
	...
	array<TreeSpriteVertex, 16> vertices;

	for (UINT i = 0; i < treeCount; ++i)
	{
		float x = MathHelper::RandF(-45.0f, 45.0f);
		float z = MathHelper::RandF(-45.0f, 45.0f);
		float y = LandUtility::GetHillsHeight(x, z);
...
void GeometryApp::BuildPSOs()
{
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&treeSpritePsoDesc, IID_PPV_ARGS(&mPSOs["treeSprite"])));
}
...
void GeometryApp::BuildMaterials()
{
	...
	auto treeSprites = make_unique<Material>();
	...
	mMaterials[treeSprites->Name] = move(treeSprites);
}
...
void GeometryApp::BuildRenderItems()
{
	...
	auto treeSpritesRitem = make_unique<RenderItem>();
	...
	mAllRitems.push_back(move(treeSpritesRitem));
}